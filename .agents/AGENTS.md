...
api/v1alpha1/                        # CRD type definitions (Kubebuilder markers)
  ...
  clustercleanpolicy_types.go        # ClusterCleanPolicy spec/status (skeleton — spec being redesigned)
...
| `ClusterCleanPolicy` | Delete resources when conditions are met on watched changes (skeleton — not yet functional, spec being redesigned) | `watchedResources`, `conditions`, `conditionRecheckInterval` |
...
- **`ClusterCleanPolicy` is a skeleton.** The CRD is registered and the reconciler syncs policies into the registry, but the processor is a no-op. The spec is being redesigned — the old `target` (engine + template) field has been removed. Do not add clean-specific logic until the new spec is finalized.
